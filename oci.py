...
class Oci:
    ...
    def connect(self):
        ...
        try:
            ...
            if response is None or "Error" in response or response == "":
                self.logger.warning(
                    "OCI::connect({host},{username}): Login failure: \n{response}\n",
                    self.ocs,
                    self.user_name,
                    response,
                )
    ...
    def parse_response(self, xml_response):
        ...
        return (
            nonce.text if nonce is not None else None,
    ...
    def send_request(self, mesg):
        ...
        if self.sock:
            try:
                ...
                self.logger.warning(
                    "send_request(): Error sending message: %s", error_message
                )
        else:
    ...
    def get_response(self, timeout=None):
        ...
        try:
            ...
            self.logger.warning(
                "get_response(): Error receiving response:", error_message
            )
            return None
